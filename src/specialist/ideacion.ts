// Especialista real de ideación: mismo patrón que src/specialist/mvp.ts
// (busca contexto relevante en rag_chunks, genera el borrador con Claude).
// Ver diseno_especialista_ideacion.md para el diseño completo, confirmado
...
const ESPECIALISTA = "ideacion";

const SYSTEM_PROMPT = `Eres el especialista de ideación de Startup-Next. Tu trabajo es generar recomendaciones concretas y accionables para la acción prioritaria que te asignó el orquestador, fundamentadas en los fragmentos recuperados de los libros base (Customer Development, Guía del Business Model Canvas).

Tu foco es la fase de ideación: validar el problema real del cliente, identificar y priorizar un segmento concreto, formular y priorizar hipótesis de valor/problema — no construcción de producto ni experimentos de MVP (eso es responsabilidad del especialista de mvp).
...
export async function runIdeacionSpecialist(
